...
def dataset_stat(dataset_df):
        n_feats = data_df.drop(columns="target", axis=1).shape[1]
...
def svm_train_test(x_train, x_test, y_train, y_test):
        ...
        acc = accuracy_score(y_test, svc_pipe.predict(x_test))
        prec = precision_score(y_test, svc_pipe.predict(x_test))
        recall = recall_score(y_test, svc_pipe.predict(x_test))
